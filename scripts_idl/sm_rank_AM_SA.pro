...
fname = data_dir+STRING('FLDAS_NOAH01_C_SA_M.A',startyr,startmo,'.001.nc',f='(a,I4.4,I2.2,a)')
...
;;;; READIN NOAH NETCDF FILES AND GET SOIL MOISTURE FOR SOUTHERN AFRICA

for i=0,nmos-1 do begin &$
   moi = startmo +i &$
   if moi gt 12 then moi = moi -12 &$
   ;data_dir = '/home/ftp_out/people/mcnally/FLDAS/FLDAS4DISC/NOAH_CHIRPSv2.001_MERRA2_EA/'
   fnames = FILE_SEARCH(data_dir,STRING('FLDAS_NOAH01_C_SA_M.A????',moi,'.001.nc',f='(a,I2.2,a)')) &$
   print, fnames &$
   nyrs = N_ELEMENTS(fnames) &$
   ;ncdf_list,fnames[0],/VARIABLES, /DIMENSIONS, /GATT, /VATT
   fid = NCDF_OPEN(fnames[0],/NOWRITE) &$
...
;if lt 0 then total in a given dimension.
n_neg = TOTAL(SMtot lt 0.0000,3) ;total the count that are lt zero
glocs = WHERE(n_neg eq 0) ;this returns the single value index
gind = ARRAY_INDICES([NX,NY],glocs,/DIMENSIONS) ;array indices helps return the position in x,y. good for using where in array.
...
grtitle = TEXT(0.5,0.955,'May Soil Moisture Rank 2017 ', ALIGNMENT=0.5,FONT_SIZE=16)
tmpgr.save, '/home/almcnall/IDLplots/SM01_Rank_MarToMay_v1.png', RESOLUTION=300
...
grtitle = TEXT(0.5,0.955,'Mar-May Soil Moisture Rank 2017', ALIGNMENT=0.5,FONT_SIZE=16)
tmpgr.save, '/home/almcnall/IDLplots/SM01_Rank_MarToMay_v2.png', RESOLUTION=300

;write out files for Chemonics
ofile = '/home/almcnall/IDLplots/SM01_Rank_MarToMay_2016_17.tif
write_tiff, ofile, ssnrnk[*,*,-1], /FLOAT

test = read_tiff(ofile, GEOTIFF=g_tags)
temp = image(test)
