pro readin_CHIRPRS_NOAH_streamflow
...
;when i want to make this more general...
...
stream = FLTARR(NX,NY,nmos,nyrs)*!values.f_nan
...
for yr=startyr,endyr do begin &$
  ...
  ;VOI = 'FloodStor_tavg' &$ ;
  VOI = 'Streamflow_tavg' &$
  flow = get_nc(VOI, ifile) &$
  stream[*,*,i,yr-startyr] = flow &$

endfor &$
endfor
Qsuf(where(Qsuf lt 0)) = !values.f_nan
Qsub(where(Qsub lt 0)) = !values.f_nan
stream(where(stream lt 0)) = !values.f_nan

Store_surf = Qsuf+Qsub

help, store_surf, stream

;RO_CHIRPS01 = RO
delvar, Qsuf, Qsub, qs, qsb, flow
