...
;; 07/06/17 use this as base to read in ECV then subset by basin
...
;;;;;;;;;;subset by basin of interest from hymap (maybe use the africa wide..);;;;;;;;;;;
...
basin25 = congrid(basin, NX, NY) & help, basin25

;looks like a pretty good match
testECV = mean(ecvcube[*,*,*,10], dimension=3, /nan) & help, testECV

;18=zambeie, karibe dam
good = where(basin25 eq 18, complement = other)
zamb_mask = basin25
zamb_mask(good) = 1
...
;limpopo=58
good = where(basin25 eq 58, complement = other)
limp_mask=basin25
limp_mask(good) = 1
...
help, zamb_mask
;temp = image(zamb_mask)

;plot the time series for the zambieze basin 
ecv_vect = reform(ecvcube, nx, ny, nyrs*nmos) & help, ecv_vect
zamb_mask396 = rebin(zamb_mask, nx, ny, 396) & help, zamb_mask396

z_ecv_ts = mean(mean(ecv_vect*zamb_mask396, dimension=1, /nan), dimension=1, /nan) & help, z_ecv_ts

;now i want to plot this with the FLDAS outputs...

temp = plot(z_ecv_ts)
;get z_SM01_ts from HYDRO_102
help, z_ecv_ts, z_sm01_ts
;write out CSV for Shrad's SRI
header = ['ECV', 'SM01']
ofile = '/home/almcnall/IDLplots/Zamb_EVC_SM01.csv'
write_csv,  ofile, z_ecv_ts,z_sm01_ts, header=header
