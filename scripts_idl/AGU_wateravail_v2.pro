...
;06/05/17 revisit for basin wide averages (water balance)
;08/31/17 revisit to make some basin avg plots w/ polygon mask not box
;12/07/17 back for AGU! this time write out tiff so kris can make nice map. i could do this for all regions and she could stich it. then 
;maybe i'd see that upward trend in burkina faso.
...
;there is some weird shift in here...[not sure what this means 12/7/17] 
...
;;trendmap looks fine, what are the units? avg annual kg/m2/s (blow up with 86400*365 ?)
;;write out as a tiff for kris, replot that and population with basin overlay

;read in a tiff to get the g-tag (use EROS data?)
;;east africa
...
ofile = '/home/almcnall/IDLplots/EA_RO_trend_1990_2017.tif'
write_tiff, ofile, reverse(trendmap,2), geotiff=g_tags, /FLOAT
...
ofile = '/home/almcnall/IDLplots/WA_RO_trend_1990_2017.tif'
write_tiff, ofile, reverse(trendmap,2), geotiff=g_tags, /FLOAT

;;southern africa
ifile = file_search('/discover/nobackup/almcnall/SHPfiles/sa_monthly_fldas_soilmoi00_10cm_tavg_1710.tif')
ingrid = read_tiff(ifile, GEOTIFF=g_tags) & print, g_tags

ifile2 = file_search('/home/almcnall/IDLplots/SA_RO_trend_1990_2017.tif')
ingrid = read_tiff(ifile2, GEOTIFF=g_tags) & print, g_tags

trendmap=ingrid
ofile = '/home/almcnall/IDLplots/SA_RO_trend_1990_2017.tif'
write_tiff, ofile, reverse(trendmap,2), geotiff=g_tags, /FLOAT
write_tiff, ofile,ingrid, geotiff=g_tags, /FLOAT

;;grab the time series for the basins of interest;;; convert units s*day*area/mm
rf = mean(mean(ro_annual*rebin(rufi_mask, nx, ny,28),dimension=1, /nan), dimension=1, /nan)* 86400*365*(255646474945)/1000  & help, rf
lv = mean(mean(ro_annual*rebin(lvb_mask, nx, ny, 28), dimension=1, /nan), dimension=1, /nan)* 86400*365*(550379339428)/1000   & help, lv

p1 = plot(rf, /buffer)
p1 = plot(lv, 'b', /buffer, /overplot)
p1.save, '/home/almcnall/IDLplots/temp.png'

;look at the timeseries for the upper tana basin
;chop everything down...why didn't i do this with the basin map??
...
;
;print, right - left + 1print, top - bot + 1
...
;;;;UPPER TANA BASIN OUTLOOKS WITH ESP;;;;;;;;
;upper tana basin mask from readin_HYMAP_basins.pro
help, utana_mask
UTTS = total(total(ro_chirps01*rebin(utana_mask, nx, ny, 12, 36),1,/nan),1,/nan) & help, utts
utts(where(utts eq 0))=!values.f_nan
UTTS_vect = reform(UTTS,12*36) & help, UTTS_vect
utts_vect(where(utts_vect eq 0))=!values.f_nan

UTTS33 = total(total(esp33*rebin(utana_mask, nx, ny, 12),1,/nan),1,/nan) & help, utts33
utts33(where(utts33 eq 0))=!values.f_nan

UTTS67 = total(total(esp67*rebin(utana_mask, nx, ny, 12),1,/nan),1,/nan) & help, utts67
utts67(where(utts67 eq 0))=!values.f_nan
;UTTS33_vect = reform(UTTS33,12*36) & help, UTTS_vect
;utts33_vect(where(utts33_vect eq 0))=!values.f_nan



;generate the date string with TIMEGEN and label_date
dummy = LABEL_DATE(DATE_FORMAT=['%Y-%M'])
time82 = TIMEGEN(START=JULDAY(1,1,1982), FINAL=JULDAY(1,1,2018), units='months')
;time82 = TIMEGEN(START=JULDAY(1,1,2016), FINAL=JULDAY(12,31,2017), units='months')

;really i should add the mean on as the last year just so i can get a good smooth without edge effects - this is why computing by hand was better.
p1 = plot(time82, UTTS_vect[0:431]*86400,xrange=[min(time82),max(time82)], xtickformat='label_date')
p1 = plot(time82, smooth(UTTS_vect[0:431],4)*86400,'m',xrange=[min(time82),max(time82)], xtickformat='label_date', /overplot)
p1 = plot(time82, smooth(UTTS_vect[0:431],18)*86400,'c',xrange=[min(time82),max(time82)], xtickformat='label_date', /overplot)
p1 = plot(time82, smooth(UTTS_vect[0:431],24)*86400,'b',xrange=[min(time82),max(time82)], xtickformat='label_date', /overplot)

;cummulative check...someday this will be useful for droughts w/ the slope.
p1 = plot(time82, total(UTTS_vect[0:431],/cumulative),xrange=[min(time82),max(time82)], xtickformat='label_date')

;;ok so i need to add on the median runoff to this time series. Get ESP_median from get_ESPmedian.pro
help, ro_chirps01
help, utana_mask ;from readin_HYMAP_basins
help, espmedian, esp33, esp67 ;2017 data...concat this to the UTTS_vect?

print, mean(ro_chirps01[*,*,*,35], /nan)
ro_chirps01[*,*,*,35] = espmedian

;;since that is hard to see, plot the outlook vs the LTM and previous yrs
;;plot the outlook compared to the longterm mean...
w = WINDOW(DIMENSIONS=[900,900]);works for EA 700x900
p1 = barplot(mean(utts,dimension=2,/nan)*86400, thick=2, /current, fill_color='w', name='average runoff')
p2 = plot(utts[*,35]*86400,'r',/overplot, thick = 3, name='median outlook')
p3 = plot(utts[0:6,35]*86400,'orange',/overplot, thick = 3, name='2017 FLDAS runoff')
p4 = plot(utts33*86400, /overplot, 'grey', thick =2, name='33rd & 67th percentile')
p4 = plot(utts67*86400, /overplot, 'grey', thick =2, name='33rd & 67th percentile')
!null = legend(target=[p1,p2,p3,p4], position=[0.2,0.3], orientation=0,shadow=0)

;would be nice to see the ESP spread here, 67th and 33rd percentile or min/max...
...
;;repeat with ET
;chnage to basin mask from box at some point?
...
roc = reform(ro_chirps01,nx,ny,nyrs*nmos) & help, roc
...
ifile = file_search('/discover/nobackup/projects/fame/MODEL_RUNS/NOAH_OUTPUT/daily/AFRICA/LDT_run/lis_input_af_elev.nc') & print, ifile
...
zmask82 = rebin(zamb_mask,NX, NY, NYRS*NMOS)
lmask82 = rebin(limp_mask,NX, NY, NYRS*NMOS)
hmask82 = rebin(hwan_mask,NX, NY, NYRS*NMOS)
...
;plot the limpopo time series, what 
;generate the date string with TIMEGEN and label_date
dummy = LABEL_DATE(DATE_FORMAT=['%Y-%M'])
time82 = TIMEGEN(START=JULDAY(1,1,1982), FINAL=JULDAY(12,31,2017), units='months')
help, roc, lmask82
Limp_TS = mean(mean(lmask82*roc, dimension=1, /nan),dimension=1, /nan) & help, limp_TS
p1 = plot(time82, Limp_TS,'g', xrange=[min(time82),max(time82)], xtickformat='label_date', name = 'Gaborone', /overplot)
