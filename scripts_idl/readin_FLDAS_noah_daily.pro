...
SM2day = FLTARR(NX,NY,31,nmos,nyrs)*!values.f_nan
...
Pday = FLTARR(NX,NY,31,nmos,nyrs)*!values.f_nan
P = FLTARR(NX,NY,nmos,nyrs)*!values.f_nan

ROday = FLTARR(NX,NY,31,nmos,nyrs)*!values.f_nan
...
for YR = startyr,endyr do begin &$
  for i=0,nmos-1 do begin &$
    ...
    for f = 0, n_elements(fnames)-1 do begin &$
      ...
      VOI = 'Rainf_tavg' &$ 
      P = get_nc(VOI, ifile) &$
      Pday[*,*,f,i,yr-startyr] = P &$
      
      VOI = 'Evap_tavg' &$
      ET = get_nc(VOI, ifile) &$
      ETday[*,*,f,i,yr-startyr] = ET &$ ;Qsb_tavg
      ...
      VOI = 'Qsb_tavg' &$
      Q2 = get_nc(VOI, ifile) &$
      ROday[*,*,f,i,yr-startyr] = Q1+Q2 &$ ;Qsb_tavg
;      
      VOI = 'SoilMoist_tavg' &$
      SM = get_nc(VOI, ifile) &$
      SM01 = SM[*,*,0] &$
